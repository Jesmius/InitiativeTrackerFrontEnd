import { apiFetch, requireAuth, renderNav, showAlert, setLoading, getRole, type UserProfile } from './api.js';
...
async function loadProfile(): Promise<void> {
  ...
  (document.getElementById('username-display') as HTMLElement).textContent = profile.username;
  (document.getElementById('email-display') as HTMLElement).textContent = profile.email || '—';
  (document.getElementById('role-display') as HTMLElement).innerHTML =
    `<span class="badge ${profile.role === 'master' ? 'badge-master' : 'badge-player'}">
      ${profile.role === 'master' ? 'Mestre de Mesa' : 'Jogador'}
    </span>`;

  (document.getElementById('email-input') as HTMLInputElement).value = profile.email || '';
}
...
document.getElementById('password-form')?.addEventListener('submit', async (e) => {
  ...
  if (res.ok) {
    ...
    const msg = (err['old_password'] as string) || 'Erro ao alterar senha.';
