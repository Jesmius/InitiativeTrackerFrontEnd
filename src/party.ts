import { apiFetch, requireRole, renderNav, showAlert, setLoading, type PartyMember } from './api.js';
...
async function loadMembers(): Promise<void> {
  ...
  if (!res.ok) { list.innerHTML = '<p class="text-danger">Erro ao carregar jogadores.</p>'; return; }

  const members = await res.json() as PartyMember[];
...
function buildCard(m: PartyMember): HTMLElement {
  ...
  `;
  div.querySelector<HTMLButtonElement>('[data-remove]')?.addEventListener('click', () => removeMember(m.id, m.player_username));
  return div;
}
...
async function removeMember(id: number, username: string): Promise<void> {
  if (!confirm(`Remover ${username} da sua lista de jogadores?`)) return;
  const res = await apiFetch(`/api/party/${id}/`, { method: 'DELETE' });
  if (res.ok || res.status === 204) loadMembers();
  else showAlert('page-alert', 'Erro ao remover jogador.', 'error');
}
