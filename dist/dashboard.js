import { apiFetch, getRole, renderNav, showAlert, setLoading } from './api.js';
requireAuth();
renderNav('dashboard');
const role = getRole();
...
async function loadCombats() {
    ...
    if (!res.ok) {
        combatList.innerHTML = '<p class="text-danger">Erro ao carregar combates.</p>';
        return;
    }
...
function buildCombatCard(c) {
  ...
  `;
    div.querySelector('[data-delete]')?.addEventListener('click', () => deleteCombat(c.id));
    return div;
}
createForm?.addEventListener('submit', async (e) => {
    ...
    if (!res.ok) {
        showAlert('create-alert', 'Erro ao criar combate.', 'error');
        return;
    }
...
async function deleteCombat(id) {
    if (!confirm('Excluir este combate? Esta ação não pode ser desfeita.'))
        return;
    const res = await apiFetch(`/api/combats/${id}/`, { method: 'DELETE' });
    if (res.ok || res.status === 204)
        loadCombats();
    else
        showAlert('page-alert', 'Erro ao excluir combate.', 'error');
}
function esc(str) {
